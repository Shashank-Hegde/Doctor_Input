...
def mapping_view_section():
    ...
    # Build row selector labels
    row_options = []
    scenario_col = None
    for cand in ["Scenario ID / Name", "Scenario ID", "Scenario"]:
    ...
    for idx, row in df.iterrows():
        label = f"Row {idx + 1}"
        if scenario_col:
            val = str(row[scenario_col]).strip()
            if val:
                label += f" – {val}"
        row_options.append((label, idx))

    row_labels = [lbl for (lbl, _) in row_options]
    selected_row_label = st.selectbox("Select a row to view:", row_labels)

    selected_idx = dict(row_options)[selected_row_label]
    row_series = df.iloc[selected_idx]

    st.markdown(f"#### Details for **{selected_row_label}** (read-only)")
    row_df = pd.DataFrame({
        "Field": row_series.index,
        "Value": row_series.values,
    })

    # View-only, scrollable
